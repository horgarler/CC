...
public interface AdministratorDashboardRepository extends AbstractRepository{
	...
	//Down
	@Query("select count(*) * 100 / (select count(*) from Item i where i.itemType = :type) from Down")
	double ratioOfItemWithDown (ItemType type);
	
	@Query("select x.quantity.currency, avg(x.quantity.amount), stddev(x.quantity.amount), min(x.quantity.amount), max(x.quantity.amount) from Down x group by x.quantity.currency")
	List<Object[]> statsQuantityOfDown();
